...
public class MySQL {
    ...
    public static void connect() {
        if (!isConnected())
            try {
                ...
                Bukkit.getConsoleSender().sendMessage(CustomAreaCoin.chatcolor("&a(!) &eMySQL Info: &aENABLED"));
    ...
    public static void close() {
        if (isConnected())
            try {
                con.close();
                Bukkit.getConsoleSender().sendMessage(CustomAreaCoin.chatcolor("&a(!) &eMySQL Info: &cDISABLED"));
            } catch (SQLException e) {
                e.printStackTrace();
            }
    }
