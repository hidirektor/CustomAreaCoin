...
public class MadenCoinAPI {
    ...
    public static void purgeKredi() {
        if(manager.config.getConfig().getBoolean("mysql-enabled")) {
            MySQL.update("DELETE FROM " + MySQL.tablename);
        } else {
            File dataFile = new File(CustomAreaCoin.getPlugin().getDataFolder(), "data.yml");
            dataFile.delete();
            manager.data = new ConfigAPI(CustomAreaCoin.getPlugin(), "data", Boolean.valueOf(true));
            manager.data.save();
        }
    }
